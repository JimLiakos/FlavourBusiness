...
namespace ThermalDotNet
{
    ...
    public class ThermalPrinter
    {
        ...
        {
            string logo = "";
            if (!File.Exists(@"C:\mono-logo.bmp"))
                return null;
            BitmapData data = GetBitmapData(@"C:\mono-logo.bmp");
            ...
            return logo + System.Text.Encoding.Default.GetString(bytes);
        }
        ...
        /// </param>
        public ThermalPrinter(IPort serialPort, byte maxPrintingDots, byte heatingTime, byte heatingInterval)
        {
        ...
        /// Serial port used by printer.
        /// </param>
        public ThermalPrinter(IPort serialPort)
        {
            _constructor(serialPort, _maxPrintingDots, _heatingTime, _heatingInterval);
        }

        private void _constructor(IPort serialPort, byte maxPrintingDots, byte heatingTime, byte heatingInterval)
        {
            this.Encoding = "ASCII";// "ibm850";
        ...
        {
            byte[] command = new byte[3] { 0x10, 0x4, 1 };
            _serialPort.Write(command, 0, 3);
            byte[] response = _serialPort.Read(1);

            return (int)response[0];

        }

        public async Task<int> GetPrinterStatusAsync()
        {
            byte[] command = new byte[3] { 0x10, 0x4, 1 };
            _serialPort.Write(command, 0, 3);
            byte[] response = await _serialPort.ReadAsync(1);
        ...
        {
            byte[] command = new byte[3] { 0x10, 0x4, 2 };
            _serialPort.Write(command, 0, 3);
            byte[] response = _serialPort.Read(1);
        ...
        public async Task<int> GetOffLineStatusAsync()
        {
            byte[] command = new byte[3] { 0x10, 0x4, 2 };
            _serialPort.Write(command, 0, 3);
            byte[] response =await _serialPort.ReadAsync(1);
        ...
        public int GetΕrrorStatus()
        {
            byte[] command = new byte[3] { 0x10, 0x4, 3 };
            _serialPort.Write(command, 0, 3);
            byte[] response = _serialPort.Read(1);
        ...
        public async Task<int> GetΕrrorStatusAsync()
        {
            byte[] command = new byte[3] { 0x10, 0x4, 3 };
            _serialPort.Write(command, 0, 3);
            byte[] response = await _serialPort.ReadAsync(1);
        ...
        {
            byte[] command = new byte[3] { 0x10, 0x4, 4 };
            _serialPort.Write(command, 0, 3);
            byte[] response = _serialPort.Read(1);
        ...
        public async Task<int> GetPaperStatusAsync()
        {
            byte[] command = new byte[3] { 0x10, 0x4, 4 };
            _serialPort.Write(command, 0, 3);
            byte[] response =await _serialPort.ReadAsync(1);
    ...
    public class IPort
    {
        ...
        public IPort()
        {
            IPAddress ipAddress = IPAddress.Parse("10.0.0.142");
            IPEndPoint remoteEP = new IPEndPoint(ipAddress, 9100);
        ...
        public object PortName { get; internal set; }

        internal byte[] Read(int length)
        {
            byte[] response = new byte[length];
            sender.Receive(response, length, SocketFlags.None);
            return response;

        }


        internal Task<byte[]> ReadAsync(int length)
        {
            byte[] buffer = new byte[length];
            var tcs = new TaskCompletionSource<byte[]>(sender);
            sender.BeginReceive(buffer, 0, length, SocketFlags.None, iar =>
            {
                var t = (TaskCompletionSource<byte[]>)iar.AsyncState;
                var s = (Socket)t.Task.AsyncState;
                try
                {
                    int readedBytes = s.EndReceive(iar);

                    t.TrySetResult(buffer);
                }
                catch (Exception exc)
                {
        ...
        internal void Write(byte[] outputBytes, int startPos, int length)
        {
            ...
            try
            {
                ...
                //byte[] command = new byte[3] { 0x10, 0x4, 1 };
                //int bytesSentp = sender.Send(command, 0, 3, SocketFlags.None);
                //byte[] response = new byte[1] { 0 };
                //sender.Receive(response, 1, SocketFlags.None);

                var bytesSent = sender.Send(outputBytes, startPos, length, SocketFlags.None);

                //command = new byte[3] { 0x10, 0x4, 1 };
                //int bytesSentpp = sender.Send(command, 0, 3, SocketFlags.None);
