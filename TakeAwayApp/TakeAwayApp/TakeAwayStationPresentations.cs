            ...
            get
            {
                if(_WatchingOrders == null)
                {
                    if (_HomeDeliveryCallCenterStation != null)
                    {
                        ...
                        _WatchingOrders =callCenterStationWatchingOrders.WatchingOrders.Select(watchingOrder => new WatchingOrderPresentation(watchingOrder, watchingOrder.MealCourses.Select(x => _MealCoursesInProgress.GetViewModelFor(x)).ToList())).ToList();
                    }
                }
